classdef Log4jConfigurator<handle
    ...
    methods (Static)
        function res = isConfigured()
            res = mxberry.log.log4j.Log4jConfigurator.getSetConfStatus();
        end
        function res = isLocked()
            res = mxberry.log.log4j.Log4jConfigurator.getSetLockStatus();
        end
        function configureSimply(logLevel,varargin)
            ...
            import mxberry.core.throwerror;
            %
            [~,prop]=mxberry.core.parseparams(varargin,[],0);
            nProp=length(prop);
            isLock = false;
            for k=1:2:nProp-1
                switch lower(prop{k})
            ...
            %
            if mxberry.log.log4j.Log4jConfigurator.isLocked()
                logger=Logger.getLogger('mxberry.log.log4j.Log4jConfigurator');
                logger.warn(['Attempt to change a locked Log4j configuration', sprintf('\n'), ...
                    mxberry.core.MExceptionUtils.meStack2String(...
                    dbstack('-completenames'),...
                    'useHyperlinks',false,'prefixStr','  ')]);
                return;
            end
            %
            ...
            %
            mxberry.log.log4j.Log4jConfigurator.getSetConfStatus(true);
            if isLock
                mxberry.log.log4j.Log4jConfigurator.lockConfiguration();
            end
        end
        function logger=getLogger(loggerName,isSuffix)
            ...
            if nargin==0||isSuffix
                ...
                [methodName,className]=...
                    mxberry.core.getcallernameext(2);
        ...
        function lockConfiguration()
            mxberry.log.log4j.Log4jConfigurator.getSetLockStatus(true);
        end
        function unlockConfiguration()
            mxberry.log.log4j.Log4jConfigurator.getSetLockStatus(false);
        end
        function res = getLastLogPropStr()
            res = mxberry.log.log4j.Log4jConfigurator.getSetLogPropStr();
        end
    end
    ...
    methods (Access=protected)
        ...
        function configureInternal(self,logPropStr,varargin)
            ...
            if isempty(logPropStr)
                ...
                %
                if ~mxberry.io.isdir(logLocPath)
                    mxberry.io.mkdir(logLocPath);
                end
                %
                java.lang.System.setProperty(...
