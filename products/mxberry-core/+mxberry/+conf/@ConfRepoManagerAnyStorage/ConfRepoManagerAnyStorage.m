classdef ConfRepoManagerAnyStorage<mxberry.core.obj.HandleObjectCloner
    ...
    methods (Access=private)
        function [SConfData,metaData]=getConfFromStorage(self,confName)
            ...
            SConfData=...
                mxberry.core.struct.updateleaves(SConfData,self.getStorageHook);
        end
        %
    end
    methods
        ...
        function putConfToStorage(self,confName,SConfData,metaData)
            SConfData=...
                mxberry.core.struct.updateleaves(SConfData,...
                self.putStorageHook);
    ...
    methods
        ...
        function self=ConfRepoManagerAnyStorage(storage,varargin)
            ...
            % Input:
            %   regular:
            %       storage: mxberry.core.cont.ondisk.IOnDiskBranchedStorage[1,1] -
            %          object reposible for maintaining a configuration
            %          storage
            %
            %   optional:
            %       patchRepoObj:
            %           mxberry.core.struct.AStructChangeTracker[1,1]
            ...
            import mxberry.core.struct.*;
            mxberry.core.checkvar(storage,...
                @(x)isa(x,...
        ...
        function selectConf(self,confName,varargin)
            ...
            [~,prop]=mxberry.core.parseparams(varargin,{'reloadIfSelected'},0);
            if ~isempty(prop)
        ...
        function isPositive=isParam(self,paramName)
            ...
            isPositive=mxberry.core.struct.structcheckpath(...
        ...
        function resVal=getParam(self,paramName,varargin)
            ...
            import mxberry.core.parseparext;
            %
            [~,~,isCacheSkipped]=parseparext(varargin,...
                {'skipCache';false;@(x)islogical(x)&&isscalar(x)},0);
            %
            if isCacheSkipped
                self.reCacheCurConf();
            end
            try
                resVal=mxberry.core.struct.structgetpath(self.getCurConf(),...
                    paramName);
                %
            catch meObj
                newMeObj=throwerror('invalidParam',...
        ...
        function setParam(self,paramName,paramValue,varargin)
            ...
            %
            %
            [~,prop]=mxberry.core.parseparams(varargin,{'writedepth'},0);
            isWriteToDisk=true;
            if ~isempty(prop)
            ...
            %
            curConfName=self.getCurConfName(); %#ok<*PROPLC,*PROP>
            [curConf,metaData]=self.getCurConf();
            SConf=mxberry.core.struct.structapplypath(curConf,paramName,...
                paramValue);
            %
        ...
        function putConf(self,confName,SConf,varargin)
            ...
            %
            %
            [restArgList,~,isSelected]=mxberry.core.parseparext(varargin,...
        ...
        function destFileName=copyConfFile(self,destFolderName,varargin)
            ...
            if isDestFile
                ...
                if ~mxberry.io.isdir(destFolderName)
                    mxberry.io.mkdir(destFolderName);
                end
        ...
        function confNameList=getConfNameListInternal(self)
            ...
            try
                ...
                if ~isempty(strfind(meObj.identifier,':badKeyValuePair'))
                    newMeObj=throwerror('badConfRepo',...
            ...
            if self.isCachedConf(confName)
                ...
                try
                    ...
                    if ~isempty(strfind(meObj.identifier,':noRecord'))
                        newMeObj=throwerror('noConfiguration',...
