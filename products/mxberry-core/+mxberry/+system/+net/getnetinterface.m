function SNetInfoVec=getnetinterface(varargin)
...
%
[reg,prop]=mxberry.core.parseparams(varargin,[],0);
if ~isempty(reg)
    mxberry.core.throwerror('wrongInput', 'Invalid regular argument');
end
nProp=length(prop);
for k=1:2:nProp-1
    switch lower(prop{k})
...
SNetInfoVec = cell2struct(cell(length(interfaceFieldsCVec), nInterfaces),...
    interfaceFieldsCVec, 1);
%
...
hardwareCVec = cellfun(@(x)formatHardwareAddress(x.getHardwareAddress()),...
    netInterfaceCVec, 'UniformOutput', false);
[SNetInfoVec.interfaceName] = deal( nameCVec{:} );
[SNetInfoVec.interfaceDisplayName] = deal( displayNameCVec{:} );
[SNetInfoVec.hardwareAddress] = deal( hardwareCVec{:} );
%
...
for iInt = 1:nInterfaces
    ...
    for iAddr = 1:nAddresses
        ...
        if strcmp(ipStruct(iAddr).ipVersion, 'IPv4')
            broadcastObj=interfaceAddress.getBroadcast();
            if ~isempty(broadcastObj)
                ipStruct(iAddr).broadcastAddress=char(broadcastObj.getHostAddress());
            else
                ipStruct(iAddr).broadcastAddress='';
            end
        end
        %
    ...
    %
    SNetInfoVec(iInt).ipAddresses = ipStruct;
end
%
if isMustHaveAddress
    SNetInfoVec( arrayfun(@(x)isempty(x.ipAddresses),SNetInfoVec) ) = [];
end
if isMustHaveHwAddress
    SNetInfoVec( arrayfun(@(x)isempty(x.hardwareAddress),SNetInfoVec) ) = [];
end
%
