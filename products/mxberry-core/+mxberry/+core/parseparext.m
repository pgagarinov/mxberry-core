function [reg,isRegSpecVec,varargout]=parseparext(args,propNameValMat,...
    varargin)
% PARSEPAREXT behaves in the same way as mxberry.core.parseparams but
...
[inpReg,inpProp]=mxberry.core.parseparams(varargin,...
...
[reg,prop]=mxberry.core.parseparams(args,propNameList,inpReg{:});
%
nRegs=length(reg);
...
[isUnique,propUResNameList]=mxberry.core.isunique(propResNameList);
if ~isUnique
    ...
    dupPropListStr=mxberry.core.cell.cellstr2expression(...
...
if isObligatoryPropVecSpec
    ...
    if any(isObligAndNotSpecVec)
        throwerror('wrongInput',['property(es) %s are obligatory but ',...
            'were not specified'],...
            mxberry.core.cell.cell2tablestr([],...
