...
%
if ~usejava('jvm')
    throwerror('wrongState:NoJvm','no Java is running');
end
...
if isempty(server)
    throwerror('wrongState:SMTPServerIndeterminate',...
        'Could not determine SMTP server');
end
if isempty(from)
    throwerror('wrongState:FromAddressIndeterminate',...
...
if any(subject == char(10)) || any(subject == char(13))
    throwerror('wrongInput:InvalidSubject',  ...
...
if numel(attachments) == 0
    ...
    for iAttachments = 1:numel(attachments)
        ...
        if isempty(fullName)
            throwerror('wrongState:CannotOpenFile', ...
...
try
    ...
    if isempty(niceError)
        ...
        throwerror('wrongState:SmtpError','%s',niceError);
    end
end
...
try
    ...
    throwerror('wrongInput:AddressError','%s',...
        stripJavaError(meObj.message));
end
...
for i = 1:length(crList)-1
    ...
    while moreOnLine
        ...
        lines{end+1,1} = nextLine(start:stop); %#ok<AGROW>
    end
end
