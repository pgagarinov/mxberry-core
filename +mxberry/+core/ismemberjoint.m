...
import mxberry.core.ismembersortableobj;
%
if nargin<2
...
if isempty(dim)
    ...
    isEqualLeftSize=mxberry.core.checksize(leftCArr{:},leftSize);
    if ~isEqualLeftSize
        throwerror('wrongInput',...
    ...
    isEqualRightSize=mxberry.core.checksize(rightCArr{:},rightSize);
    if ~isEqualRightSize
        throwerror('wrongInput',...
...
for iRow=1:nLeftComp
    ...
    if isnumeric(rightMat)||islogical(rightMat)||ischar(rightMat)
        [rightMat,~,rightIndMat(:,iRow)]=mxberry.core.uniquerows(rightMat,false,prop{:});
        [~,leftIndMat(:,iRow)]=mxberry.core.ismemberrows(leftMat,rightMat,false,prop{:});
    else
        ...
        if isCharStr
            ...
            if nCols==1
                ...
                [rightIndColMat,~,rightIndMat(:,iRow)]=mxberry.core.uniquerows(rightIndColMat,true,prop{:});
                [~,leftIndMat(:,iRow)]=mxberry.core.ismemberrows(leftIndColMat,rightIndColMat,true,prop{:});
            end
        else
            ...
            if isBothOpaque
                if isSortMethodDefined
                    ...
                    [~,leftIndMat(:,iRow)]=...
                        mxberry.core.ismemberbyfunc(leftCArr{1},rightCArr{1});
                end
                ...
                if ~isequaln(leftUniqMat,rightUniqMat)
                    if isLeftSorted&&isRightSorted
                        ...
                        [~,curInd]=mxberry.core.ismemberbyfunc(...
...
elseif nLeftComp==1
    ...
    if isInd
        [isThereVec,indThereVec]=mxberry.core.ismemberrows(leftIndMat,...
            rightIndMat,true,prop{:});
    else
        isThereVec=mxberry.core.ismemberrows(leftIndMat,rightIndMat,...
            true,prop{:});
    end
end
%
