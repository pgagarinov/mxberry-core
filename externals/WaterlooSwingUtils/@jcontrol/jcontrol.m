...
%   21.08.11 Remove uipanel host
%            Remove transparency support - not used as unavailable on Win
...
elseif strcmp(class(Parent),'jcontrol')
    error('JCONTROL:obsolete', 'Using a jcontrol as a parent of a jcontrol is no longer supported');
elseif strcmp(class(Parent),'hgjavacomponent')
    error('JCONTROL:obsolete', 'Using an hgjavacomponent as a parent of a jcontrol is no longer supported');
% Modified 05.11.08
...
elseif isa(Style, 'function_handle')
    ...
    % Fall back to the user supplied object and try that.
    % Some HG2 java objects can return 0 on isjava : TODO check this
    javaobject=Style;
end
