...
if isempty(path2KeepList)
    ...
    [field2KeepList,~,indRep]=unique(fieldSeqCell(:,1));
    %
    ResStruct=mxberry.core.struct.fieldfilterstruct(InpStruct,field2KeepList);
    %
    ...
    for iField=1:nUniqueFields
        ...
        if size(subFieldSeqCell,2)>1
            ...
            ResCell=num2cell(mxberry.core.struct.pathfilterstruct(InpSubStruct,...
                subPath2KeepList));
            ResSubStruct=struct(fieldName,ResCell);
            %
            ResStruct=mxberry.core.struct.binaryunionstruct(ResStruct,...
                ResSubStruct,@(x,y)y);
        end
    end
end
...
for iPath=1:nPath
    ...
    curPathCell=[repmat({'.'},1,indLastNotEmpty);curPathCell]; %#ok<AGROW>
    curPath=horzcat(curPathCell{:});
    pathList=[pathList,{curPath}]; %#ok<AGROW>
end
%
end
