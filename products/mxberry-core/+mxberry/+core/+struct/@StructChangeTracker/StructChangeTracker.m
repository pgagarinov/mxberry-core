classdef StructChangeTracker<mxberry.core.struct.AStructChangeTracker
    ...
    methods
        ...
        function [SInput,lastVersion]=applyAllLaterPatches(self,SInput,startRev)
            ...
            elseif lastVersion<startRev
                mxberry.core.throwerror('badRepoState',...
                    ['inconsistent '...
    ...
    methods(Access=private)
        function [funcHandleList,revNumVec]=findPatchFuncList(self,startRev,endRev,isInclusiveVec)
            ...
            if any(isPatchVec)
                ...
                %
                [SResList]=regexp(funcNameList,...
                    '^patch_(?<patchver>\d*)','names');
                nMatchesVec=cellfun('length',SResList);
                if any(nMatchesVec>1)
                    mxberry.core.throwerror('badState',...
                        ['Oops, we shouldn''t be here, regular ',...
