...
for iField=1:nFields
    ...
    if isCommonField(iField)
        for iFieldOp=1:nFieldOps
            ...
            %
            [resTmpCell{:}]=cellfun(fieldOp,...
                reshape({leftStruct.(fieldName)},sizeStructVec),...
                reshape({rightStruct.(fieldName)},sizeStructVec),...
                'UniformOutput',false);
            %
            updaterescell();
            %
    ...
    elseif isLeftField(iField)
        for iFieldOp=1:nFieldOps
            ...
            %
            [resTmpCell{:}]=cellfun(leftFieldOp,reshape(...
                {leftStruct.(fieldName)},sizeStructVec),...
                'UniformOutput',false);
            %
            updaterescell();
            %
        ...
        for iFieldOp=1:nFieldOps
            ...
            %
            [resTmpCell{:}]=cellfun(rightFieldOp,reshape(...
                {rightStruct.(fieldName)},sizeStructVec),...
                'UniformOutput',false);
            %
            updaterescell();
            %
...
varargout{:}=[];
mxberry.core.throwerror('illegalOperation',...
    ['common fields cannot be processed by default, ',...
    'binary operation required']);
end
