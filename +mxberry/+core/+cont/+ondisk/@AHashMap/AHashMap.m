classdef AHashMap<mxberry.core.cont.ondisk.IOnDiskBranchedStorage
    ...
    methods
        ...
        function self=AHashMap(varargin)
            ...
            if self.isHashedPath
                storageBranchKey=mxberry.core.hash(storageBranchKey);
            end
            %
            if ~isStorageLocSpec
            ...
            if ~strcmpi(self.storageFormat,'none')
                if ~mxberry.io.isdir(self.storageLocation)
                    %check that a directory if exists, containts only mat files
                    mxberry.io.mkdir(self.storageLocation);
                else
        ...
        function put(self,keyList,valueObjList,varargin)
            ...
            varargin(isnCellVec)=cellfun(@(x){x},varargin(isnCellVec),...
                'UniformOutput',false);
            %
            if ~mxberry.core.checksize(keyList,size(valueObjList))
        ...
        function [valueList,varargout]=get(self,keyList,varargin)
            ...
            %
            import mxberry.core.parseparext;
            [~,~,isUniformOutput]=parseparext(varargin,...
                {'uniformOutput';true;@(x)islogical(x)&&isscalar(x)},0);
            %
            if ~iscell(keyList)
                keyList={keyList};
            end
        ...
        function keyList=getKeyList(self)
            ...
            %
            [~,keyList]=cellfun(@(x)self.checkKey(x),fileNameList,...
                'UniformOutput',false);
        end
        ...
        function removeAll(self)
            ...
            %
            %
            storageLocation=self.getStorageLocation(); %#ok<*PROP>
    ...
    methods (Access=protected)
        function [isPositive,keyStr]=checkKey(self,fileName)
            ...
            if isPositive
                ...
                try
                    ...
                    meCauseObj=throwerror('unknownFailure',...
                        'failed to load file %s',fileName);
                    meObj=meObj.addCause(meCauseObj);
                    rethrow(meObj);
                end
                ...
                if isPositive
                    ...
                    keyStr='';
                    throwwarn('incorrectKeyValueFile',...
        ...
        function putOne(self,keyStr,valueObj) %#ok<INUSD>
            import mxberry.core.throwwarn;
            fullFileName=self.genfullfilename(keyStr);
            try
    ...
    methods (Access=protected)
        function fileName=genfilename(self,inpStr)
            ...
            if self.isHashedKeys
                inpStr=mxberry.core.hash(inpStr);
            end
            %
            if ~isempty(inpStr)
