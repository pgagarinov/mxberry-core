...
import mxberry.core.uniquesortableobj;
%
if nargin<1
...
if isempty(indDim)
    ...
    isEqualSize=mxberry.core.checksize(inpCArr{:},inpSizeVec);
    if ~isEqualSize
        throwerror('wrongInput',...
...
for iRow=1:nInp
    ...
    if isnumeric(inpMat)||islogical(inpMat)||ischar(inpMat)
        [~,~,indMat(:,iRow)]=mxberry.core.uniquerows(inpMat,false,prop{:});
    elseif isstruct(inpMat)
        curMat=reshape(mxberry.core.num2cell(...
        ...
        elseif isNumCell||isStructCell||isCharCell
            for iCol=1:nCols
                ...
                if isStructCell
                    ...
                    [~,~,curMat]=mxberry.core.uniquerows(curIndMat,true,prop{:});
                ...
                [sizeVars,~,curIndMat(:,1)]=mxberry.core.uniquerows(...
                ...
                for iVar=1:nVars
                    ...
                    if isNumCell
                        [~,~,curIndMat(isVar,2)]=mxberry.core.uniquerows(curMat,isIntCell,prop{:});
                    else
                        curMat=[{vertcat(curMat{:,1})},mxberry.core.num2cell(curMat(:,2:end),1)];
                        [~,~,curIndMat(isVar,2)]=uniquejoint(curMat,1);
                    end
                end
                if nVars>1
                    [~,~,indColMat(:,iCol)]=mxberry.core.uniquerows(curIndMat,true,prop{:});
                else
        ...
        if nCols==1
            ...
            [~,~,indMat(:,iRow)]=mxberry.core.uniquerows(indColMat,true,prop{:});
        end
    end
end
...
elseif nInp==1
    ...
    if isBackwardInd
        [~,indRight2LeftVec,indLeft2RightVec]=mxberry.core.uniquerows(indMat,true,prop{:});
    else
        [~,indRight2LeftVec]=mxberry.core.uniquerows(indMat,true,prop{:});
    end
end
%
if isempty(indDim)
