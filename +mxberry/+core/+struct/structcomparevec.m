...
%          Vadim Kaushansky, PhD <vkaushanskiy@gmail.com> $
...
if ~mxberry.core.checksize(SX,SY,[1 1])
...
if ~isEqual
    ...
    reportStrList={['Field names are different, left-right:',...
        cell2tablestr('',fieldXMinusYList,'|'),...
        ', right-left: ',cell2tablestr('',fieldYMinusXList,'|')]};
    return;
end
%
nFields=length(fieldXList);
reportAddList=cell(1,nFields);
for iField=1:nFields
    ...
    if ~isEqualCur
        ...
        reportAddList{iField}=reportStrCurList;
    end
end
reportStrList=[reportStrList,reportAddList{:}];
end
...
elseif isstruct(x)
    [isEqual,reportStr]=mxberry.core.struct.structcompare(x,y,absTol,relTol);
    if ~isEqual
        return;
    end
