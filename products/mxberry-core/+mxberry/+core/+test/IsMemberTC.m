...
classdef IsMemberTC < mxberry.unittest.TestCase
    methods
    ...
    methods (Test)
        function self=testIsMemberJointAdv(self)
            import mxberry.core.ismemberjoint;
            leftList={{1,2,nan},{1,2,3,nan}};
            rightList={{1,2,nan},{1,2,3,nan},{1,2,2,nan}};
            [isThereVec,indThereVec]=ismemberjoint({leftList},...
                {rightList},2);
            self.verifyEqual(isThereVec,[true,true]);
            self.verifyEqual(indThereVec,[1,2]);
        end
        function testByInputSize(self)
            N_ISMEMBER_OUTS=2;
            N_UNIQUE_OUTS=3;
            %%
            ...
            checkMasterIsMember();
            %
            checkIsMember(@mxberry.core.ismembercellstr);
            ...
            function checkMasterUnique()
                checkUniq(@mxberry.core.uniquebyfunc);
                checkUniq(@(x)mxberry.core.uniquebyfunc(x,@isequal));
                checkUniq(@(varargin)mxberry.core.uniquebyfunc(...
                ...
                checkUniq(@mxberry.core.unique);
            end
            %
            function checkMasterIsMember()
                checkIsMember(@mxberry.core.ismemberbyfunc);
                checkIsMember(@(varargin)mxberry.core.ismemberbyfunc(...
                    varargin{:},@isequal));
                checkIsMember(@mxberry.core.ismember);
            ...
            function varargout=fCallIsMemberJoint(varargin)
                ...
                [outList{:}]=mxberry.core.ismemberjoint(inpArgList{:});
                varargout=outList;
            end
            %
            function varargout=fCallUniqueJoint(varargin)
                outList=cell(1,N_UNIQUE_OUTS);
                [outList{:}]=mxberry.core.uniquejoint(varargin(1),...
                    varargin{2:end});
        ...
        function testEqCallCount(self)
            ...
            function checkMaster(nObjs)
                ...
                self.verifyEqual((nObjs-1)*(nObjs-1),nIsMemberJointCalls);
                %
                ...
                function objVec=createObjVec(nElems,varargin)
                    objVec=mxberry.core.test.aux.EntityFactory.create(...
                        1:nElems,varargin{:});
                end
            end
        end
        ...
        function self=test_ismemberjoint_empty(self)
            ...
            self.verifyEqual(true,isempty(isMember));
            self.verifyEqual(true,isempty(indMember));
            %
            [isMember,indMember]=ismemberjoint({zeros(10,0),false(10,0)},{zeros(5,0),false(5,0)},1);
            self.verifyEqual(true,isequal(isMember,true(10,1)));
            self.verifyEqual(true,isequal(indMember,repmat(5,10,1)));
        end
        function self=test_ismemberjoint(self)
            ...
            self.runAndCheckError(...
                '[isMemberVec,indMemberVec]=mxberry.core.ismemberjoint(leftCell,rightCell,1);',...
                'wrongInput');
            %
            leftCell={[1 2;11 22],...
                {'a','b';'aa','bb'},...
            ...
            rightCell={[],{},[],{}};
            self.runAndCheckError(...
                '[isMemberVec,indMemberVec]=mxberry.core.ismemberjoint(leftCell,rightCell,2);',...
                ':wrongInput');
            %
        ...
        function self=test_ismemberjoint_enum(self)
            import mxberry.core.ismemberjoint;
            enumVal=mxberry.core.test.aux.TestEnum.Alpha;
            arrayList={[1;2],[enumVal;enumVal]};
            [a,b]=ismemberjoint(arrayList,arrayList,1);
            self.verifyEqual(true,isequal(a,[true;true]));
            self.verifyEqual(true,isequal(b,[1;2]));
        end
        function self=test_uniquejoint_ext(self)
            ...
            self.verifyEqual(true,isSorted);
            %
            nRows=200;
            ...
            self.verifyEqual(nUniqueRows,numel(indForward2Vec));
            indForwardVec=indBackward1Vec(indForward2Vec);
            self.verifyEqual(true,~any(diff(sort(indForwardVec))==0));
            outputCell1=cellfun(@(x)x(indForwardVec),outputCell1,'UniformOutput',false);
            for iElem=1:nElems
                self.verifyEqual(true,...
                    isequaln(outputCell1{iElem},outputCell2{iElem}));
            end
            self.verifyEqual(true,isequal(indBackward1Vec,indForwardVec(indBackward2Vec)));
            self.verifyEqual(true,isequal(indForwardVec(indBackward2Vec(indForward1Vec)),(1:nUniqueRows).'));
        end
        function self=test_ismemberjointwithnulls(self)
            ...
            superCheck(2);
            self.runAndCheckError(...
                '[isMemberVec,indMemberVec]=mxberry.core.ismemberjointwithnulls(leftCell,leftIsNullCell,rightCell,rightIsNullCell,1);',...
                ':wrongInput');
            %
            ...
            self.runAndCheckError(...
                '[isMemberVec,indMemberVec]=mxberry.core.ismemberjointwithnulls(leftCell,rightCell,2);',...
                ':wrongInput');
            %
            rightCell=ethRightCell;
            ...
            function superCheck(dim)
                [isMemberVec,indMemberVec]=...
                    mxberry.core.ismemberjointwithnulls(leftCell,leftIsNullCell,rightCell,rightIsNullCell,dim);
                check();
            end
            function check()
                self.verifyEqual(true,...
        ...
        function self=test_uniquejoint_performance(self)
            ...
            function checkTime(inpMat,nRuns)
                import mxberry.core.uniquejoint;
                MAX_TOLERANCE=0.95;
                outMat1=[];
                indForwardVec1=[];
                ...
                indForwardVec2=[];
                indBackwardVec2=[];
                time1=self.runAndMeasureTime(@run1,...
                    'nRuns',nRuns,'useMedianTime',true);
                time2=self.runAndMeasureTime(@run2,...
                    'nRuns',nRuns,'useMedianTime',true);
                self.verifyEqual(true,isequal(outMat1,outMat2));
                self.verifyEqual(true,isequal(indForwardVec1,indForwardVec2));
                self.verifyEqual(true,isequal(indBackwardVec1,indBackwardVec2));
                time1=min(time1,time2);
                time2=self.runAndMeasureTime(@run3,...
                    'nRuns',nRuns,'useMedianTime',true);
                self.verifyEqual(true,isequal(outMat1,outMat2));
                self.verifyEqual(true,isequal(indForwardVec1,indForwardVec2));
                self.verifyEqual(true,isequal(indBackwardVec1,indBackwardVec2));
                ...
                messageStr=sprintf(...
                    ['Ratio error %f between chosen and ',...
                    'mininal exceeds maximal one %f'],...
                    curTolerance,MAX_TOLERANCE);
                self.verifyEqual(true,curTolerance<MAX_TOLERANCE,messageStr);
                function run1()
                    [outMat1,indForwardVec1,indBackwardVec1]=...
                        mxberry.core.uniquejoint({inpMat},1,'optimized');
                end
                function run2()
                    [outMat2,indForwardVec2,indBackwardVec2]=...
                        mxberry.core.uniquejoint({inpMat},1,'standard');
                end
                function run3()
                    [outMat2,indForwardVec2,indBackwardVec2]=...
                        mxberry.core.uniquejoint({inpMat},1);
                end
            end
        end
        function self=test_ismemberjoint_performance(self)
            ...
            function checkTime(inpMat1,inpMat2,nRuns,maxTol)
                import mxberry.core.ismemberjoint;
                MAX_TOLERANCE_DEFAULT=1;
                if nargin<4
                ...
                time2=self.runAndMeasureTime(@run2,...
                    'nRuns',nRuns,'useMedianTime',true);
                self.verifyEqual(true,isequal(isMemberVec1,isMemberVec2));
                self.verifyEqual(true,isequal(indMemberVec1,indMemberVec2));
                time1=min(time1,time2);
                time2=self.runAndMeasureTime(@run3,...
                    'nRuns',nRuns,'useMedianTime',true);
                self.verifyEqual(true,isequal(isMemberVec1,isMemberVec2));
                self.verifyEqual(true,isequal(indMemberVec1,indMemberVec2));
                curTolerance=max(max(time1/time2,time2/time1)-1,0);
                messageStr=sprintf(['Ratio error %f between chosen ',...
                    'and mininal exceeds maximal one %f'],...
                    curTolerance,maxTol);
                self.verifyEqual(true,...
                    curTolerance<maxTol,messageStr);
                function run1()
                    [isMemberVec1,indMemberVec1]=...
                        mxberry.core.ismemberjoint({inpMat1},...
                        {inpMat2},1,'optimized');
                end
                function run2()
                    [isMemberVec2,indMemberVec2]=...
                        mxberry.core.ismemberjoint({inpMat1},...
                        {inpMat2},1,'standard');
                end
                function run3()
